...
{
  $connect = connexionBdd();
  $requete="SELECT  l.date,l.Message,u.nom_user from log_users l inner join users u on u.id = l.id_users where id_users = $id ORDER by l.date DESC;";
...
function add_log($message,$id)
{
  ...
  $connect = connexionBdd();
  $requete="INSERT INTO log_users (date,Message,id_users) VALUES ('".$date."','".$message."',$id);";
  $ok=$connect->query($requete);
